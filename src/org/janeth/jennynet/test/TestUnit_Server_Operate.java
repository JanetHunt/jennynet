...
//                     con.getParameters().setAlivePeriod(0);
   ...
   @Test
   public void test_addRemove_serverListener () {
...
//   @Test
//   public void test_addRemove_connection () {
...
//   }
   ...
   @Test
   public void test_accepting_LISTEN () {
      TestServer sv1 = null, sv2 = null, sv3 = null;
      int h;
      ...
      try {
         ...
         assertTrue("client con was not added to server's list",
               sv1.getConnections().length == 1);
         assertNotNull("false client con was added to server's list",
               sv1.getConnection(scon1.getUUID()));
         assertTrue("client con was not signalled as ADDED to listener",
               listen1.getAddedCounter() == 1);
         assertTrue("false client con was added to listener's list", listen1
               .getAddedCons().contains(scon1));
         ...
         assertTrue("client con was not signalled as ADDED to listener",
               listen1.getAddedCounter() == 2);
         assertTrue("false client con was added to listener's list", listen1
               .getAddedCons().contains(scon2));
         ...
         assertTrue("signalled con was falsely started",
               listen1.getConnectedCounter() == 2);
         assertTrue("client con was signalled as ADDED to listener",
               listen1.getAddedCounter() == 2);
         assertFalse("client con was added to listener's ADDED list", listen1
               .getAddedCons().contains(scon3));

         // test server condition
         assertTrue("client con was added to server's list",
               sv1.getConnections().length == 2);
         assertNull("client con was added to server's list",
               sv1.getConnection(scon3.getUUID()));
         assertTrue("listener error counter > 0",
               listen1.getErrorCounter() == 0);
   ...
   @Test
   public void test_accepting_ACCEPT () {
      TestServer sv1 = null, sv2 = null, sv3 = null;
...
//         sv1.getParameters().setAlivePeriod(0);
         ...
         sv1.setSignalMethod(SignalMethod.Accept);
         sv1.start();
         acceptThread.start();
         assertTrue("server acceptThread init error",
               acceptThread.getAvailableCounter() == 0);

         ServerConnection scon1, scon2, scon3;
         Client con1, con2, con3;
         con1 = new Client(new InetSocketAddress("localhost", 55555));
         con1.getParameters().setAlivePeriod(3000);
         con1.connect(0, sv1.getSocketAddress());
         sleep(300);
         ...
         assertTrue("false value client signalled counter: " + h, h == 1);
         assertTrue("signalled con was not actually started",
               acceptThread.getConnectedCounter() == 1);
         ...
         assertTrue("falsely signalled client to listener",
               listen1.getAvailableCounter() == 0);
         assertTrue("client con was not signalled as ADDED to listener",
               listen1.getAddedCounter() == 1);
         assertTrue("client con was not added to listener's list", listen1
               .getAddedCons().contains(scon1));
         ...
         assertTrue("false value client signalled counter: " + h, h == 2);
         assertTrue("signalled con was not actually started",
               acceptThread.getConnectedCounter() == 2);
         assertTrue("false client signalled",
               scon2.getRemoteAddress().equals(con2.getLocalAddress()));
         ...
         assertTrue("signalled con was falsely started",
               acceptThread.getConnectedCounter() == 2);
         assertTrue("client con was falsely signalled as ADDED to listener",
               listen1.getAddedCounter() == 2);
         ...
         assertNull("client con was added to server's list",
               sv1.getConnection(scon3.getUUID()));
         assertTrue("acceptThread error counter > 0",
               acceptThread.getErrorCounter() == 0);
   ...
   @Test
   public void test_remote_closing () {
      TestServer sv1 = null;
   ...
   @Test
   public void test_connection_timeout () throws Exception {
      ...
      // connection attempt to server (must fail with
   ...
   @Test
   public void test_close_server () throws IOException {
      Server sv1 = null;
      ...
      try {
         ...
         assertTrue("server has not listed connection",
               sv1.getConnections().length == 1);
         sc1 = (ServerConnection)sv1.getConnections()[0];
         assertTrue("server connection is not connected", sc1.isConnected());
         assertTrue("illegal server closed event",
               listen1.getServerClosedCounter() == 0);
         ...
         assertTrue("server lost connection after close",
               sv1.getConnections().length == 1);
         sc1 = (ServerConnection)sv1.getConnections()[0];
         ...
         // close all connections in server closed state (just normal)
         sv1.closeAllConnections();
         Util.sleep(50);
         assertTrue("server fails to close connection", sc1.isClosed());
         assertTrue("server fails to remove connection",
               sv1.getConnections().length == 0);
   ...
   @Test
public void test_close_all_connections () throws IOException {
   Server sv1 = null;
